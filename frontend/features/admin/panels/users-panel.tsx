...
export function UsersPanel({ users, mutate }: { users: SafeUser[]; mutate: Mutate }) {
  const [createRole, setCreateRole] = useState<SafeUser["role"]>("STUDENT");
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        ...
        <div className="divide-y divide-ink/10">
          {users.map((user) => (
            ...
            >
              ...
              {user.role === "STUDENT" && (
                <select className="field" name="parentId" defaultValue={user.parentId ?? ""} required>
      ...
      <form
        ...
        action={(formData) => {
          const payload = {
            ...
            lastName: String(formData.get("lastName")),
            role: String(formData.get("role")),
            subscriptionTier: String(formData.get("subscriptionTier") || "FOUNDATION"),
            parentId: String(formData.get("parentId") || "") || null
          };
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          {createRole === "STUDENT" ? (
            <>
              <input className="field" name="username" placeholder="Student username" required />
          ...
          <select className="field" name="role" value={createRole} onChange={(event) => setCreateRole(event.target.value as SafeUser["role"])}>
            <option value="STUDENT">Student</option>
            <option value="PARENT">Parent</option>
            <option value="ADMIN">Admin</option>
          </select>
